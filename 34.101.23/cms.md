...
# 7 <span id="Data"></span>Неструктурированные данные
...
подписывающих сторон (см. <A href="Signed2"> 8.2 </a> ).
...
подписанных данных. В 8.5, 8.6, 8.7 описываются процессы хэширования, 
...
которых действуют другие правила (см. 8.1).
...
«хэш-значение» (см. 15.2 и 15.3). Поэтому значения данных атрибутов,
...
## 8.7 <span id="Signed7"></span>Проверка ЭЦП
...
`RecipientInfo`, которые определяют формат конвертованных данных. В 9.4, 
9.5 описываются процессы шифрования данных и ключей.

## 9.2 <span id="Enveloped2"></span>Тип EnvelopedData
...
## 9.3 <span id="Enveloped3"></span>Тип RecipientInfo
...
Процесс шифрования ключа шифрования данных описан в 9.5.

- `encryptedKey` определяет зашифрованный ключ шифрования данных для
получателя.

### 9.4.1 <span id="Enveloped41"></span>Тип KeyAgreeRecipientInfo
...
### 9.4.2 <span id="Enveloped42"></span>Тип `KEKRecipientInfo`
...
#9.4.3 <span id="Enveloped43"></span>Тип `PasswordRecipientInfo`
...
#9.4.4 <span id="Enveloped44"></span>Тип `OtherRecipientInfo`
...
## 9.6 <span id="Enveloped6"></span>Процесс шифрования ключей
...
Хэширование производится в соответствии с 8.5 для случая, когда
...
## 11.2 <span id="Encrypted2"></span>Тип EncryptedData
...
# 12 <span id="Auth"></span>Аутентифицируемые данные
...
## 12.3 <span id="Auth3"></span>Вычисление имитовставки
...
«тип содержимого» и «хэш-значение», определенные в 15.2 и 15.3. При этом на
...
дополняются до требуемой длины по правилам, определенным в 9.5. Эти правила
...
(см. `authAttrs` в 12.2 и 13.2), неаутентифицируемыми (см. `unauthAttrs` в 
12.2 и 13.2) и незашифрованными (см. `unprotectedAttrs` в 9.2 и 11.2).

Атрибуты описываются с помощью типа `Attribute` АСН.1 (см. 8.4). Синтаксис
...
## 15.3 <span id="Attr3"></span>Хэш-значение
...
подписывается (см. 8.5) или для которого вычисляется имитовставка 
(см. 12.3). Для подписываемых данных алгоритм хэширования выбирается 
...
## 15.4 <span id="Attr4"></span>Время подписания
...
## 15.5 <span id="Attr5"></span>Контрподпись
...
# Приложение А  <span id="AppendixA"></span>(обязательное)
# Модули АСН.1
...
# <span id="Biblio"></span>Библиография

[1] <span id="ISO/IEC 9594-8:2008"></span>ISO/IEC 9594-8:2008 Information 
...
[2] <span id="Message_Syntax"></span>Housley R. Cryptographic Message Syntax 
(CMS). Request for Comments: 5652, 2009 (Синтаксис криптографических 
сообщений) 

[3] <span id="Message_Syntax1"></span>Housley R. Cryptographic Message 
...
[4] <span id="Message_Syntax3"></span>Turner S., Housley R. Cryptographic 
...
[5] <span id="PFOK"></span>Проект руководящего документа Республики Беларусь 
