...
<span id="ГОСТ_34_974"></span>ГОСТ 34.974-91
...
информацией об их владельце. Формат структуры определен в [{1]](#ISO/IEC9594-8:2008) (версия 1)
и [{2]](#Message_Syntax) (версия 2).
...
имитозащиты и шифрования к произвольным данным. Стандарт основан на [[2]](#Message_Syntax) и
...
выработкой ЭЦП данные (см. [8.4](#Signed4)). На этом же шаге собираются сертификаты и
...
подписываемыми атрибутами. Хэширование подробно описано в [8.5](#Signed5). Используемый
...
подписываемых данных соответственно (см. [15.2](#Attr2) и [15.3](#Attr3)). Однако атрибут «тип
...
«хэш-значение» (см. [15.2](#Attr2) и [15.3](#Attr3)). Поэтому значения данных атрибутов,
т.е. тип и хэш-значение подписываемых данных, неявно учитываются в
окончательном хэш-значении. Атрибут «тип содержимого» НЕ ДОЛЖЕН
...
данные ДОЛЖНЫ хэшироваться по правилам, определенным в [8.5](#Signed5). Для признания
...
`RecipientInfo`, которые определяют формат конвертованных данных. В [9.4](#Enveloped4), 
...
[9.4](#Enveloped4).Одни и те же алгоритм и ключ шифрования данных используются 
...
данного компонента определен в [9.3](#Enveloped3). Набор ДОЛЖЕН содержать по крайней мере
один элемент.
...
«тип содержимого» и «хэш-значение», определенные в [15.2](#Attr2) и [15.3](#Attr3). При этом на
вход алгоритма выработки имитовставки подается закодированное с помощью
отличительных правил кодирования значение компонента `authAttrs`. Компонент
`authAttrs` должен кодироваться для вычисления имитовставки как отдельное
от `AuthenticatedData` значение типа SET OF Atrribute. Это значит, что при
кодировании тег `IMPLICIT [2]` ДОЛЖЕН быть заменен на тег `SET OF`. Таким
образом, обработке при вычислении имитовставки подвергается октет тега `SET
OF`, дополненный октетами длины и содержимого кодового представления
`authAttrs`.
...
правилам согласно [12.3](#Auth3), и ключ имитозащиты, предаваемый в компоненте
...
которых вычисляется имитовставка, определяются в [12.3](#Auth3). Если отправитель
включает аутентифицируемые атрибуты, то имитовставка вычисляется от
значения компонента `authAttrs` согласно [12.3](#Auth3). Кроме того, при наличии
...
правил. Тип данного компонента определен в [12.2](#Auth2). Данный компонент НЕ ДОЛЖЕН
содержать атрибут «хэш-значение». В разделе [15](#Attr) описаны некоторые
...
дополняются до требуемой длины по правилам, определенным в [9.5](#Enveloped5). Эти правила
действуют только в том случае, когда идет речь о выравнивании данных на
блок из *k* &lt; 256 октетов.
...
При шифровании ключей на вход алгоритма шифрования подается
непосредственно ключ шифрования данных, без атрибутов и информации о
получателе. Любой из описанных в [9.3](#Enveloped3) методов управления ключами может быть
...
был использован отправителем (см. [9.4.2](#Enveloped42), [9.4.3](#Enveloped43)). Следует избегать
...
данными. Атрибуты могут быть подписываемыми (см. `signedAttrs` в [8.4](#Signed4)),
неподписываемыми (см. `unsignedAttrs` в [8.4](#Signed4)), аутентифицируемыми
(см. `authAttrs` в [12.2](#Auth2) и [13.2](#AuthEnveloped2)), неаутентифицируемыми (см. `unauthAttrs` в 
[12.2](#Auth2) и [13.2](#AuthEnveloped2)) и незашифрованными (см. `unprotectedAttrs` в [9.2](#Enveloped2) и [11.2](#Encrypted2)).

Атрибуты описываются с помощью типа `Attribute` АСН.1 (см. [8.4](#Signed4)). Синтаксис
...
значения компонента `signature` типа `SignerInfo` (см. [8.4](#Signed4)) данное число
представляется строкой октетов, которая ДОЛЖНА определяться следующим
образом:
