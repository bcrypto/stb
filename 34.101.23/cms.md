...
значения компонента `signature` типа `SignerInfo` (см. 8.4) данное число
...
ДОЛЖЕН применяться метод, описанный в 9.5.
...
(см. 8.4) данная строка естественным образом представляется строкой из 48,
72 или 96 октетов.
...
`KeyTransRecipientInfo` (см. 9.4.1).
...
применяться метод, описанный в 9.5.
...
заданных в [5], описаны в СТБ П 34.101.50 (приложение В).

Определенный в [5] протокол одностороннего формирования общего ключа МОЖНО
