...
СОК СШВ ДОЛЖЕН содержать расширение `ExtKeyUsageSyntax` (см. СТБ 34.101.19
(пункт 6.2.1)) с единственным компонентом типа `KeyPurposeId`, и этот компонент
должен принимать значение `id-kp-timeStamping`. Расширение `ExtKeyUsageSyntax`
ДОЛЖНО быть критическим.
