...
`certificatesHashIndex`, `crlsHashIndex`, `unsignedAttrsHashIndex`. Этот же
...
Компонент `unsignedAttrsHashIndex` содержит список хэш-значений, вычисленных от
элементов типа `Attribute`, содержащихся в компонентах `unsignedAttrs` всех
объектов типа `SignerInfo` на момент выработки штампа времени
`ArchiveTimeStamp`. Для каждого элемента типа `Attribute` хэшируется 
объединение одного из значений в его компоненте `Attribute.attrValues` 
со значением в компоненте `Attribute.attrType`. Другие хэш-значения не могут 
