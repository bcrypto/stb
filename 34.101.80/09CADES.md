...
Отнесение атрибута к подписанту, документу или подписи 
...
В компоненте `issuer` типа `IssuerSerial` указывается имя издателя сертификата.
Для сертификата открытого ключа компонента `issuer` содержит один элемент типа
`GeneralName` с опцией `directoryName`, в которой указывается значение `issuer`
типа `Name` сертификата открытого ключа.
<!--- TODO: Для атрибутных сертификатов ... --->
...
Компонент `spDocument` содержит либо закодированный документ
...
При создании значения типа `CrlValidatedID` значение `crlHash` 
...
Атрибут МОЖЕТ содержать ссылки на полный набор СОС
