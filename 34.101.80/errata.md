...
| Пункт 9.5.1, абзац 2 | Синтаксис атрибута описывается типом `ATSHashIndexV2`:<br>`ATSHashIndexV2 ::= SEQUENCE {` | Синтаксис атрибута описывается типом `ATSHashIndexV3`:<br>`ATSHashIndexV3 ::= SEQUENCE {` |
| Пункт 9.5.1, абзац 7 | Другие хэш-значения не могут содержаться в списке `unsignedAttrsHashIndex`. | Для каждого элемента типа `Attribute` хэшируется объединение одного из значений в его компоненте `Attribute.attrValues` со значением в компоненте `Attribute.attrType`. Другие хэш-значения не могут содержаться в списке `unsignedAttrsHashIndex`. |
