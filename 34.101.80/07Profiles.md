...
РЭЦП класса СAdES-B-LT ДОЛЖНА содержать атрибут `SignedData.crls.crl`, если 
среди аттестатов отзыва есть СОС, и должна содержать атрибут 
`SignedData.crls.other`, если среди аттестатов есть OCSP-ответ. 
