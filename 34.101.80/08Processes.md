...
цепочке сертификатов, построенной на шаге 3, сохраняют стойкость в момент
...
то перейти к подшагу г). Иначе — закончить проверку с полученными на шаге 2 
...
Во-вторых, если иное не оговорено политикой проверки, ДОЛЖНЫ выполняться 
ограничения СТБ 34.101.82 относительно полей `accuracy` и `ordering` 
компонентов `TSTInfo` штампов, выпущенных одной или несколькими службами. В 
случае обнаружения противоречий закончить проверку с вердиктом  `INDETERMINATE` 
и статусом `TIMESTAMP_ORDER_FAILURE`. 
