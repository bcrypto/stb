...
сертификатов и спи-сков отозванных сертификатов инфраструктуры открытых 
ключей 
...
ДОЛЖЕН» выражает абсолютный за-прет на выполнение соответствующих 
...
удостоверить фак-ты владения данными, существования данных, 
...
ЭД и СОК в одной инфраструктуре и выпускать аттестат за-верения в другой. 
...
- заверение факта владения данными (cpd, от англ. Certificate of Possession of Data);
...
доку-мент может быть признан недействительным, если в нем недостает 
...
запрос и ответ мо-гут защищаться, например, инкапсулироваться в контейнеры 
...
(например, через браузер), а ответ – другим (на-пример, по электронной 
почте). 
...
формировании ссылки ДОЛЖЕН использоваться алгоритм хэши-рования, 
...
хэширования и его па-раметры. Компонент digest задает непосредственно 
хэш-значение. 
...
`statusString` (вердикт в удобной для вос-приятия форме), `failInfo` 
...
политики проверки. Тип определяется следущим образом: 
...
Если компонент отсутствует, то действи-тельность проверяется на текущий 
...
[1]  <a name="Biblio1"></a> Adams C., Sylvester P., Zolotarev M., 
Zuccherato R. Internet X.509 Public Key Infrastructure. Data Validation 
and Certification Server Protocols. Request for Comments: 3029, 2001 
(Интернет-инфраструктура открытых ключей X.509. Протоколы проверки и заверения данных)


[2] <a name="Biblio2"></a> Freed N., Borenstein N. Multipurpose Internet 
Mail Extensions (MIME). Part One: Format of Internet 
Message Bodies. Request for Comments: 2045, 1996 
(Универсальные расширения почты интернет (MIME). Часть 1: Формат частей интернет-сообщения)
