...
Ключ, с помощью которого подписывается ответ, необязательно должен быть тем же
ключом, которым подписан сертификат. Эмитент сертификата явно делегирует OCSP
серверу право подписи, выдавая ему сертификат, содержащий специальный 
идентификатор в расширении `ExtKeyUsageSyntax`. Данный сертификат ДОЛЖЕН 
напрямую выдаваться серверу OCSP компетентным УЦ.
...
открытый ключ указывается в компоненте `subjectPublicKey` типа `BIT STRING` 
контейнера `SubjectPublicKeyInfo`. Хэшируется кодовое представление 
`subjectPublicKey`, за исключением тега, длины и числа неиспользуемых битов;
...
- `responderID` – определяет OCSP сервер; принимает значения типа `ResponderID`:
    ...
    В качестве алгоритма хэширования ДОЛЖЕН использоваться алгоритм, 
    определенный в запросе;

- `producedAt` – время создания ответа (см. [5.5](#common5)), представлено типом
`GeneralizedTime`, определенным в СТБ 34.101.19;

- `responses` – список ответов по всем проверяемым сертификатам;

- `responseExtensions` – необязательные расширения, которые могут обрабатываться
...
СОС. Ответы, в которых компонент `nextUpdate` содержит более раннее значение, 
чем время локальной системы, ДОЛЖНЫ считаться ненадежными. Если сервер не 
задает значение компонента `nextUpdate`, то тем самым он сообщает, что 
актуальная информация об отзыве доступна в любое время;
...
В ответ по невыпущенному сертификату НЕ ДОЛЖНЫ включаться расширения `CrlID` 
(п. [6.3.2](#status32)) и `CRLEntry` (п. [6.3.5](#status35)). Кроме этого, при 
выборе в `certStatus` значения типа `RevokedInfo` его компоненты ДОЛЖНЫ быть 
заданы следующим образом:

- в `revocationReason` указывается причина отзыва `certificateHold`;
- в компоненте `revocationTime` устанавливается дата 01.01.1970.
...
ответов OCSP ДОЛЖНО реализовываться путем включения идентификатора 
`id-kp-OCSPSigning` в расширение сертификата `ExtKeyUsageSyntax`, которое, в 
свою очередь, должно содержаться в сертификате OCSP сервера. Данный сертификат
ДОЛЖЕН выпускаться тем УЦ, который выпустил проверяемый сертификат. При 
...
исключительных случаях, поскольку клиенты не обязаны распознавать сервер с таким
сертификатом как уполномоченный.
...
- содержит идентификатор `id-kp-OCSPSigning` в расширении `ExtKeyUsageSyntax` 
...
некритическое. Содержимое указанного расширения должно быть определено как 
NULL. УЦ, выдающие сертификаты с таким расширением, должны понимать, что 
компрометация ключа OCSP сервера будет столь же серьезна, как и компрометация 
ключа УЦ, используемого для подписи СОС, по крайней мере на время срока 
действия выданного сертификата. УЦ следует выдавать сертификаты с указанным 
расширением на очень короткий срок и часто их обновлять;
...
способом. Детально способ указания любого из двух данных механизмов описан в 
СТБ 34.101.19;
...
`extnValue` имеет тип `OCTET STRING` и содержит информационную часть 
расширения.
...
СОС), номера (номера СОС) или времени (времени, когда был создан 
соответствующий СОС). Для ссылки на СОС может использоваться расширение с 
идентификатором `id-pkix-ocsp-crl`. Информационной частью расширения является 
кодовое представление значения типа
